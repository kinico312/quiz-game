...
void print_questions_matrix() {
    ...
    while (getline(file, line)) {
        ...
        if (row.size() >= 3 && !row[0].empty()) {
            // Åñëè òåìà èçìåíèëàñü
            if (row[0] != currentTheme) {
                currentTheme = row[0];

                if (currentChar > 'Z') {
                    cout << "Îøèáêà: ñëèøêîì ìíîãî òåì! Ìàêñèìóì 26." << endl;
                    break;
                }
    ...
    cout << "==============================================" << endl;
    cout << "               ÒÅÌÛ È ÁÀËËÛ" << endl;
    cout << "==============================================" << endl;

    int theme_number = 0;  // íà÷èíàåì ñ 0
    for (map<char, pair<string, vector<string>>>::iterator it = themes.begin(); it != themes.end(); it++) {
        ...
        theme_number++;  // óâåëè÷èâàåì íîìåð òåìû
    }
